...
JDOguess = 0; %% originally set to 0 
...
for i = 1:length(BMS.SDN)
    ...
    inonan = ~isnan(BMS.ustar); 
        %1 if not NaN
        %0 if NaN 
        
    if inonan(i)==1 % if ustar is no NaN, then do calcs    
        ustar = BMS.ustar(i); % single value per timestep 
        d = 0.19; %height of coral (similar to canopy height of a forest) - should be below pump 2 height 
        a = [JDOguess, C1guess];
